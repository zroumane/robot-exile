...
const sendTweet = async (tweet) => {
  ...
  if (user) {
    let channel = await client.guild.channels.cache.get(user.channel);
    if (channel?.isText()) channel.send(`https://twitter.com/${user.name}/status/${tweet.id_str}`).catch(console.error);
  }
};
